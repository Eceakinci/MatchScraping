...
OUTPUT_DIR_CLEAN = os.getenv("OUTPUT_DIR_CLEAN_TEAMS")
...
def scrape_teamlist(page, team_id, output_dir_raw):
    ...
    for pl in scoring_list:
        ...
        team_data.append({
            ...
            "name": name,
            "position": position,
            "team": page.locator(".vbw-team_header_name").all_inner_texts()
        })
...
def scrape_all_matches(team_ids, base_url, output_dir_raw):
    with sync_playwright() as p:
        ...
        for team_id in team_ids:
            url = f"{base_url}/{team_id}/players"
            page.goto(url)
            page.wait_for_selector(".vbw-o-table__body")
            scrape_teamlist(page, team_id, output_dir_raw)
        browser.close()
...
if __name__ == "__main__":
    ...
    if os.listdir(OUTPUT_DIR_RAW) and (not os.path.exists(OUTPUT_DIR_CLEAN) or not os.listdir(OUTPUT_DIR_CLEAN)):
        os.makedirs(OUTPUT_DIR_CLEAN, exist_ok=True)
        cleaner.clean_all_csv_files(f'{OUTPUT_DIR_RAW}/*.csv', OUTPUT_DIR_CLEAN)